...
```
*Bar charts -> for the moment, to graph ordinal and nominal variables*
```{r}
...
```{r}
ggplot(NHANESdata, aes(x=Race))+geom_bar(fill = "maroon") + labs(title="Race", x="Race", y="counts")
Race_table <- with(NHANESdata, table(Race))
prop.table(Race_table)*100
Race_barchart <- as.data.frame(prop.table(Race_table)*100)
ggplot(Race_barchart, aes(Race, Freq))+geom_col(fill = "maroon")
```
...
ggplot(NHANESdata, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "stack") + labs(title = "stacked bar chart", x="BMIcategories by gender", y="counts")
ggplot(NHANESdata, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "stack") + facet_wrap(~ Race) + labs(title = "stacked bar chart", x="BMIcategories by gender", y="counts")
ggplot(NHANESdata, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "stack") + facet_grid(~ Race) + labs(title = "stacked bar chart", x="BMIcategories by gender", y="counts")
#grouped bar chart:
#ggplot(data, aes(x=Independent variable, fill=dependent variable)) + geom_bar(position = "dodge") + labs(title="", x="Gender", y="Frequency")
ggplot(NHANESdata, aes(x=BMIcat, fill=Gender)) + geom_bar(position="dodge") + labs(title="grouped bar chart", x="BMI categories by gender", y="counts")
```
*Facet_wrap gives the multiple graphs depending on the race^
*Facet_grid gives the different graphs but also stacks them with each other^^
...
ggplot(BMI_gender, aes(x=reorder(Gender, mean), y=mean))+geom_bar(stat="identity", fill="darkgreen") +geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=0.2)
labs(x="Gender", 
     y="", 
     title="Mean BMI", 
...
```
*Histograms and box plots are usually used for continuous variables/data*
```{r}
...
ggplot(NHANESdata, aes(x=BMI))+
  ...
  labs(title="Colored Histogram with 30 Bins", x="BMI", y="Count")
```
*amount of bins should be close to the square root of observations*
```{r}
...
ggplot(NHANESdata, aes(x=BMI, y=..density..))+geom_histogram(bins=20, color="white", fill="steelblue")+
  scale_y_continuous(labels=scales::percent)+labs(title="BMI", y="percent", x="BMI")
```

d: Create a histogram of a different continuous variable:
```{r}
ggplot(NHANESdata, aes(x=ArmCircumf, y=..density..))+
  geom_histogram(bins=20, color="white", fill="darkgreen")+
  scale_y_continuous(labels=scales::percent)+
  labs(title="Arm Circumference", y="percent", x="Arm Circumference")
```
...
ggplot(NHANESdata, aes(x="", y=DR1TPROT))+
  geom_boxplot()+
  labs(y="Daily Protein Consumption in Grams", x="", title="Box Plot")
```
...
ggplot(NHANESdata, aes(x=Race, y=DR1TPROT))+
  ...
  labs(x="Race", y="Daily Protein Intake", title="Daily Protein Consumption in Grams")
```
